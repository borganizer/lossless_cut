...
#       maintained:
#       python2 fixes
#       video filename handling (strip unallowed characters)
# 3.0.0 
#       python3 version
#       removed Unicode string literals
...
APPDIR = os.path.dirname(\
        os.path.realpath(__file__)).replace('importcode', '')
#
CONFIG_DIR = '%s/.mythtv' \
    % os.path.expanduser("~")
CONFIG_FILE = '%s/lossless_cut.cfg' % CONFIG_DIR
INIT_CONFIG_FILE = '%simportcode/init_config.cfg' % APPDIR
INIT_DVB_SUBTITLE_CONFIG_FILE = '%simportcode/init_dvb_subtitle.cfg' % APPDIR
INIT_REMOVE_RECORDING_CONFIG_FILE = \
            '%simportcode/init_remove_recording.cfg' % APPDIR
INIT_MKVMERGE_USER_SETTINGS_CONFIG_FILE = \
            '%simportcode/init_mkvmerge_user_settings.cfg' % APPDIR
INIT_ERROR_DETECTION_CONFIG_FILE = \
            '%simportcode/init_error_detection.cfg' % APPDIR
INIT_PROJECTX_INI_FILE = '%simportcode/init_ProjectX.ini' % APPDIR
PROJECTX_INI_PATH = '%(workpath)s/%(recorded_name)s.ini'
#
APPENDTO_FORMAT = '%d:%d:%d:%d'
...
VIDEO_TRACK_DELAY = ' --sync 0:%d'
#
## String which identifies track info from mkvmerge --identity command
TRACK_ID = 'Track ID'
## Various commands and argument sets
ADD_SRT_CMD = '-o "%(workpath)s/%(recorded_name)s_tmp.mkv" "%(recordedfile)s"'
EXTRACT_DVB_SUBTITLES = '''-Djava.awt.headless=true -jar "%(projectx_jar_path)s" -ini "%(projectx_ini_path)s" -out "%(workpath)s" -name "%(recorded_name)s" "%(recordedfile)s"'''
CLEAR_CUTLIST = '--clearcutlist --chanid %(chanid)s --starttime "%(SQL_starttime)s"'
CLEAR_SKIPLIST = '--clearskiplist --chanid %(chanid)s --starttime "%(SQL_starttime)s"'
GEN_CUTLIST = '--gencutlist --chanid %(chanid)s --starttime "%(SQL_starttime)s"'
CUTS_CMD = '-o "%(workpath)s/%(recorded_name)s-%%04d.mkv" %(strip_args)s --split parts:%(split_list)s "%(sourcefile)s"'
CONCERT_CUTS_CMD = '-o "%(segment_path)s/%(segment_filename)s.mkv" %(strip_args)s --split parts:%(split_list)s "%(sourcefile)s"'
#
START_CUT_CMD = '%(strip_args)s --split parts:%(split_list)s "%(sourcefile)s"'
CONVERT_CMD = '%s -o "%%s" --title "%%s" --attachment-description "%%s" "%%s" &>>"%%s"'
GEN_GET_CUTLIST_CMD = '--chanid %(chanid)s --starttime "%(SQL_starttime)s"'
#
## Command line utilities
MKVMERGE = 'mkvmerge'
MKVMERGE_MIN_VERSION = '5.7'
MKVTOOLNIX_DOWNLOADS_URL = 'https://mkvtoolnix.download/downloads.html'
MKVTOOLNIX_SOURCE_URL = 'https://mkvtoolnix.download/source.html'
MEDIAINFO = 'mediainfo'
MEDIAINFO_MIN_VERSION = 'v0.7.5'
MEDIAINFO_XML = '--Output=OLDXML "%s"'
MEDIAINFO_URL = 'http://mediainfo.sourceforge.net/en/Download'
MEDIAINFO_PPA_URL = 'https://launchpad.net/~shiki/+archive/mediainfo'
#
## Subtitle track extracting utilities:
MYTHCCEXTRACTOR_ARGS = '--verbose -i "%(recordedfile)s"'
BIT_ARCH_64 = '%ssubtitle/%s64bit'
BIT_ARCH_32 = '%ssubtitle/%s32bit'
CCEXTRACTOR = 'ccextractor'
ISO639_2_LANG_CODE_FILE = '%simportcode/ISO639-2_language_code.txt' % APPDIR
#
## SQL statements used for processing a lossless cut recording
SQL_GET_GENRE = "SELECT genre FROM `programgenres` WHERE chanid=%(chanid)s AND starttime='%(SQL_progstart)s' LIMIT 1;"
#
## SQL statements to collect information on the device that recorded the video
SQL_GET_CARDID = "SELECT `cardtype`, `defaultinput`, `videodevice`, `audiodevice`, `hostname` FROM `capturecard` WHERE `cardid` = %d"
SQL_GET_CARDINFO = "SELECT `cardid`, `displayname` FROM `cardinput` WHERE `sourceid` = %d"
SQL_GET_SOURCEID = "SELECT `sourceid`  FROM `channel` WHERE `chanid` = %d"
#
...
SQL_GET_OR_INSERT = {
    ...
    'get_sql': "SELECT * FROM %(table)s WHERE chanid=%(chanid)s AND starttime='%(SQL_start)s';",
    'delete_sql': "DELETE FROM %(table)s WHERE chanid=%(chanid)s AND starttime='%(SQL_start)s';",
    'insert_sql': {
        'recorded': {
            24: [
        ...
        'recordedseek': {
            'All': [
                    ...
                    "INSERT INTO `mythconverg`.`recordedseek` (`chanid`, `starttime`, `mark`, `offset`, `type`) VALUES (%(chanid)s, '%(starttime)s', %(mark)s, %(offset)s, %(type)s);"
            ],
        },
        'recordedmarkup': {
            'All': [
                    ['chanid', 'starttime', 'mark', 'type', 'data'],
                    "INSERT INTO `mythconverg`.`recordedmarkup` (`chanid`, `starttime`, `mark`, `type`, `data`) VALUES (%(chanid)s, '%(starttime)s', %(mark)s, %(type)s, %(data)s);"
            ],
        },
        'program': {
            24: [
                ['chanid', 'starttime', 'endtime', 'title', 'subtitle', 'description', 'category', 'category_type', 'airdate', 'stars', 'previouslyshown', 'title_pronounce', 'stereo', 'subtitled', 'hdtv', 'closecaptioned', 'partnumber', 'parttotal', 'seriesid', 'originalairdate', 'showtype', 'colorcode', 'syndicatedepisodenumber', 'programid', 'manualid', 'generic', 'listingsource', 'first', 'last', 'audioprop', 'subtitletypes', 'videoprop'],
                "INSERT INTO `mythconverg`.`program` (`chanid`, `starttime`, `endtime`, `title`, `subtitle`, `description`, `category`, `category_type`, `airdate`, `stars`, `previouslyshown`, `title_pronounce`, `stereo`, `subtitled`, `hdtv`, `closecaptioned`, `partnumber`, `parttotal`, `seriesid`, `originalairdate`, `showtype`, `colorcode`, `syndicatedepisodenumber`, `programid`, `manualid`, `generic`, `listingsource`, `first`, `last`, `audioprop`, `subtitletypes`, `videoprop`) VALUES ('%(chanid)s', '%(starttime)s', '%(endtime)s', '%(title)s', '%(subtitle)s', '%(description)s', '%(category)s', '%(category_type)s', '%(airdate)s', '%(stars)s', '%(previouslyshown)s', '%(title_pronounce)s', '%(stereo)s', '%(subtitled)s', '%(hdtv)s', '%(closecaptioned)s', '%(partnumber)s', '%(parttotal)s', '%(seriesid)s', '%(originalairdate)s', '%(showtype)s', '%(colorcode)s', '%(syndicatedepisodenumber)s', '%(programid)s', '%(manualid)s', '%(generic)s', '%(listingsource)s', '%(first)s', '%(last)s', '%(audioprop)s', '%(subtitletypes)s', '%(videoprop)s');"
            ],
            25: [
                ['chanid', 'starttime', 'endtime', 'title', 'subtitle', 'description', 'category', 'category_type', 'airdate', 'stars', 'previouslyshown', 'title_pronounce', 'stereo', 'subtitled', 'hdtv', 'closecaptioned', 'partnumber', 'parttotal', 'seriesid', 'originalairdate', 'showtype', 'colorcode', 'syndicatedepisodenumber', 'programid', 'manualid', 'generic', 'listingsource', 'first', 'last', 'audioprop', 'subtitletypes', 'videoprop'],
                "INSERT INTO `mythconverg`.`program` (`chanid`, `starttime`, `endtime`, `title`, `subtitle`, `description`, `category`, `category_type`, `airdate`, `stars`, `previouslyshown`, `title_pronounce`, `stereo`, `subtitled`, `hdtv`, `closecaptioned`, `partnumber`, `parttotal`, `seriesid`, `originalairdate`, `showtype`, `colorcode`, `syndicatedepisodenumber`, `programid`, `manualid`, `generic`, `listingsource`, `first`, `last`, `audioprop`, `subtitletypes`, `videoprop`) VALUES ('%(chanid)s', '%(starttime)s', '%(endtime)s', '%(title)s', '%(subtitle)s', '%(description)s', '%(category)s', '%(category_type)s', '%(airdate)s', '%(stars)s', '%(previouslyshown)s', '%(title_pronounce)s', '%(stereo)s', '%(subtitled)s', '%(hdtv)s', '%(closecaptioned)s', '%(partnumber)s', '%(parttotal)s', '%(seriesid)s', '%(originalairdate)s', '%(showtype)s', '%(colorcode)s', '%(syndicatedepisodenumber)s', '%(programid)s', '%(manualid)s', '%(generic)s', '%(listingsource)s', '%(first)s', '%(last)s', '%(audioprop)s', '%(subtitletypes)s', '%(videoprop)s');"
        ...
        'programgenres': {
            'All': [
                    ...
                    "INSERT INTO `mythconverg`.`programgenres` (`chanid`, `starttime`, `relevance`, `genre`) VALUES ('%(chanid)s', '%(starttime)s', '%(relevance)s', '%(genre)s');"
            ],
        },
        'oldrecorded': {
            24: [
...
DEFAULT_CONFIG_SETTINGS = {
    ...
    'export_path': '/path to an export directory',
    'keep_log': 'false',
    'logpath': '/tmp',
...
#
CONCERT_CUT_DEFAULT_FORMAT = '%SEGNUMPAD% - %TITLE%: %SUBTITLE%'
#
FORMAT_SUP_VALUES = '''%(font_point_size)s,%(background_alpha)s,%(yoffset)s,%(xoffset)s,%(xwidth)s,%(h_unused)s,%(vertical)s,%(yoffset2)s,%(maxlines)s,%(unknown_1)s,%(unknown_2)s'''
# lxml XPath filters used to parse out video details
# from XML produced by mediainfo utility
TRACKS_XPATH = '//track[@type = $track_type]'
ELEMENTS_XPATH = './*[local-name() = $element]/text()'
#
...
'if="%(recordedfile)s" of="%(bug_sample)s" skip=%(sample_startblock)s bs=1 count=25321472'
#
...
'http://www.mythtv.org/wiki/Lossless_Cut#Unsupported_Recording_Devices'
WIKI_SUPPORTED_DEVICE_TABLE_URL = \
'http://www.mythtv.org/wiki/Lossless_Cut#Supported_Recording_Devices'
WIKI_FIRST_COLUMN = '|-\n'
WIKI_MANUFACTURER_COLUMN = '|Manufacturer Name\n'
WIKI_MODEL_COLUMN = '|Device Model\n'
WIKI_ROW = '|%(recorders_cardtype)s\n'
WIKI_TABLE_COLUMN = '|%s\n'
#
## Duration and track delay time, regex strings:
DURATION_REGEX = {
    # 1h 10mn 25s 650ms
    'hrs': re.compile(''' ([0-9]+)h[^\\/]*$''', re.UNICODE),
    'mins': re.compile(''' ([0-9]+)mn[^\\/]*$''', re.UNICODE),
    'secs': re.compile(''' ([0-9]+)s[^\\/]*$''', re.UNICODE),
    'millsecs': re.compile(''' ([0-9]+)ms[^\\/]*$''', re.UNICODE),
    'mins1': re.compile('''^(.+?)[ \._\-]\[?([0-9]+)mn[^\\/]*$''', re.UNICODE),
    'secs1': re.compile('''^(.+?)[ \._\-]\[?([0-9]+)s[^\\/]*$''', re.UNICODE),
    'millsecs1': re.compile('''^(.+?)[ \._\-]\[?([0-9]+)ms[^\\/]*$''', re.UNICODE),
    'hrs2': re.compile('''([0-9]+)h[^\\/]*$''', re.UNICODE),
    'mins2': re.compile('''([0-9]+)mn[^\\/]*$''', re.UNICODE),
    'secs2': re.compile('''([0-9]+)s[^\\/]*$''', re.UNICODE),
    'millsecs2': re.compile('''([0-9]+)ms[^\\/]*$''', re.UNICODE),
    'hrs3': re.compile('''(-[0-9]+)h[^\\/]*$''', re.UNICODE),
    'mins3': re.compile('''(-[0-9]+)mn[^\\/]*$''', re.UNICODE),
    'secs3': re.compile('''(-[0-9]+)s[^\\/]*$''', re.UNICODE),
    'millsecs3': re.compile('''(-[0-9]+)ms[^\\/]*$''', re.UNICODE),
}
#
